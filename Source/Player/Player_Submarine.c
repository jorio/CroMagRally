...
static float    sideForce[MAX_PLAYERS] = {0};       // Persistent sideways force during evasion
    ...
    if (stuckTimer[player] <= 0.0f && avoidTurn == 0)
    {
		float oldX = gPlayerInfo[player].analogSteering.x;
		float newX = fabsf(oldX) - gFramesPerSecondFrac * 10.0f;	// lerp abs steering towards zero
		if (newX < 0.1f)
			newX = 0;
		newX *= (oldX >= 0) ? 1 : -1;	// restore sign
		gPlayerInfo[player].analogSteering.x = newX;
    }
