...
[kNeed_RearView			] = { { SC(TAB)		, 0				}, 0			, { CAPLUS(TRIGGERLEFT)	, CBNULL()				} },
[kNeed_Forward			] = { { SC(UP)		, SC(W)			}, 0			, { CB(A)				, CB(DPAD_UP)			} },
[kNeed_Backward			] = { { SC(DOWN)	, SC(S)			}, 0			, { CB(B)				, CB(DPAD_DOWN)			} },
[kNeed_Left			    ] = { { SC(LEFT)	, SC(A)			}, 0			, { CBNULL()			, CB(DPAD_LEFT)			} },
[kNeed_Right		    ] = { { SC(RIGHT)	, SC(D)			}, 0			, { CBNULL()			, CB(DPAD_RIGHT)		} },
