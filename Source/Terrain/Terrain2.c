...
long				row,col;
	...
	Alloc_2d_array(Byte, gTerrainShadowFlags, gTerrainTileDepth+1, gTerrainTileWidth+1);

	for (row = 0; row <= gTerrainTileDepth; row++)
		for (col = 0; col <= gTerrainTileWidth; col++)
			gTerrainShadowFlags[row][col] = 0;
	...
	dot = -OGLVector3D_Dot(&up,&gGameViewInfoPtr->lightList.fillDirection[0]);
	dot = 1.0f - dot;
	...
	for (i = 0; i < gNumTerrainItems; i++)
	{
		...
		from.x = (int)(*gMasterItemList)[i].x;
		from.y = (int)(*gMasterItemList)[i].y;
		...
		for (t = 1.0; t > 0.0f; t -= 1.0f / (length/TERRAIN_POLYGON_SIZE))
		{
			...
			for (ro = -.5; ro <= .5; ro += .5)
			{
				for (co = -.5; co <= .5; co += .5)
				{
					...
					gTerrainShadowFlags[row][col] = 0xff;				// set flag
